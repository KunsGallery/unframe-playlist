        ...
        <AnimatePresence mode="wait">
          ...
          {playerView === 'lyrics' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="w-full h-[60vh] flex flex-col overflow-y-auto no-scrollbar relative scroll-smooth px-6 lg:px-12"
              onClick={() => setPlayerView('cover')}
            >
              {parsedLyrics.length > 0 ? (
                  ...
                  {parsedLyrics.map((lyric, idx) => {
                    ...
                    if (parsedLyrics.length === 1 && lyric.time === 0) {
                      return (
                        <p
                          key={idx}
                          className="text-xl lg:text-4xl text-center font-bold text-zinc-300 leading-loose whitespace-pre-wrap"
                        >
                          {lyric.text}
                        </p>
                      );
                    ...
                    return (
                      ...
                      >
                        <p
                          className={`text-xl lg:text-3xl font-black tracking-tight leading-tight transition-all duration-700 ${
                            isActive
                ...
                <div className="flex-1 flex items-center justify-center h-full">
                  <span className="text-zinc-600 font-bold italic text-lg">
                    가사가 등록되지 않은 음원입니다.
                  </span>
                </div>
              )}
                ...
                {publicTracks.map((track, idx) => {
                  ...
                  return (
                    <div
                      key={track.id}
                      id={`track-${track.id}`}
                      onClick={() => playTrack(idx, publicTracks)}
                    ...
                    >
                      <div className="w-12 h-12 rounded-xl overflow-hidden shrink-0 relative">
                        <img
                          src={track.image}
                          className={`w-full h-full object-cover ${isCurrent && isPlaying ? 'opacity-50' : ''}`}
                          alt=""
                        />
                        {isCurrent && isPlaying && (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-black uppercase truncate ${isCurrent ? 'text-[#004aad]' : 'text-white'}`}>
                          {track.title}
                        </p>
                      ...
                      {isCurrent && (
                        <div className="w-2 h-2 rounded-full bg-[#004aad] shadow-[0_0_10px_#004aad]" />
                      )}
                    </div>
                  );
                })}
      ...
      <div className="p-8 pb-12 lg:pb-16 w-full max-w-2xl mx-auto space-y-8 bg-linear-to-t from-black via-zinc-950/90 to-transparent relative z-10">
        <div className="flex items-end justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm lg:text-lg font-bold text-[#004aad] uppercase tracking-widest mt-2 truncate">
              {currentTrack.artist}
            </p>
          </div>
          ...
          <button
            ...
            className={`p-3 rounded-full transition-all ${
              isLiked ? 'bg-red-500/10 text-red-500' : 'bg-white/5 text-white'
            }`}
          >
        ...
        <div className="flex items-center justify-between px-2">
          <button
            ...
            className={`p-3 transition-colors ${isShuffle ? 'text-[#004aad]' : 'text-zinc-500 hover:text-white'}`}
          >
          ...
          <button
            onClick={togglePlay}
            className="w-24 h-24 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shadow-2xl"
          >
            {isPlaying ? (
              isBuffering ? (
          ...
          <button
            onClick={toggleLoop}
            className={`p-3 transition-colors ${loopMode > 0 ? 'text-[#004aad]' : 'text-zinc-500 hover:text-white'}`}
          >
            {loopMode === 2 ? (
        ...
        <div className="flex items-start justify-between pt-6 border-t border-white/5 opacity-90 px-4">
          <button
            onClick={() => setPlayerView(playerView === 'list' ? 'cover' : 'list')}
            className={`flex flex-col items-center gap-2 transition-colors ${
              playerView === 'list' ? 'text-[#004aad]' : 'text-zinc-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-2 transition-colors ${
              playerView === 'lyrics' ? 'text-[#004aad]' : 'text-zinc-400 hover:text-white'
            }`}
          >
          ...
          <div className="relative flex flex-col items-center gap-2">
            <button
              ...
              className={`flex flex-col items-center gap-2 transition-colors ${
                isVolumeOpen ? 'text-[#004aad]' : 'text-zinc-400 hover:text-white'
              }`}
            >
              {isMuted || volume === 0 ? (
            ...
            {isVolumeOpen && (
              <div className="absolute bottom-[120%] left-1/2 -translate-x-1/2 w-48 bg-zinc-900 border border-white/10 rounded-2xl p-4 shadow-2xl">
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="flex-1 appearance-none h-1.5 bg-white/20 rounded-full outline-none
