...
const Admin = ({ isAdmin, user, signInWithPopup, tracks, playlists, db, appId, setToastMessage, setAuthError }) => {
  ...
  // 초기 데이터 로드 (Featured & Site Config)
  useEffect(() => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      console.error("유저 로드 에러:", e);
      setAuthError("유저 정보를 불러올 권한이 없거나 경로가 잘못되었습니다."); 
    } finally { 
  ...
  const handleStickerToggle = async (targetUid, stickerId, isGiving) => {
    try {
      ...
      // 로컬 상태 즉시 반영
      setAllUsers(p => p.map(u => u.id === targetUid ? { 
        ...u, 
      ...
      setAuthError("지급에 실패했습니다. 유저 데이터 유무를 확인하세요."); 
    }
  };
    ...
    try {
      if (editingId) {
        ...
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'tracks'), { 
          ...newTrack, isHidden: false, createdAt: Date.now() 
        });
        setToastMessage("트랙 등록 성공 🚀");
      }
      handleCancelEdit();
    } catch (err) {
      setAuthError("저장 오류가 발생했습니다.");
    }
  };
  ...
  const handleToggleVisibility = async (track) => {
    ...
    try {
      ...
      setToastMessage(track.isHidden ? "트랙 공개 👁️" : "트랙 숨김 🚫");
    } catch (err) {
      setAuthError("상태 변경 실패");
    }
  };
    ...
    try {
      if (editingPlaylistId) {
        ...
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'playlists'), { 
          ...newPlaylist, createdAt: Date.now() 
        });
  ...
  const toggleTrackInPL = (trackId) => {
    setNewPlaylist(prev => {
      ...
      return { 
        ...prev, 
        trackIds: exists ? prev.trackIds.filter(id => id !== trackId) : [...prev.trackIds, trackId] 
      };
    });
  };
  ...
  const handleSaveAllConfig = async () => {
    ...
    try {
      ...
      setToastMessage("전체 설정이 성공적으로 반영되었습니다 ✨");
    } catch (err) {
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-32 lg:pt-40 px-6 lg:px-8 container mx-auto pb-32 lg:pb-40 relative z-20 min-h-screen">
        ...
        {!isAdmin ? (
            <div className={glass + " p-16 lg:p-32 rounded-[3rem] text-center space-y-8"}>
              ...
              <button onClick={signInWithPopup} className="bg-white text-black px-12 lg:px-20 py-4 lg:py-6 rounded-full font-black uppercase text-xs lg:text-sm tracking-widest shadow-2xl">Verify Admin Access</button>
            </div>
        ) : (
            <>
                        ...
                        <motion.div key="users" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid lg:grid-cols-12 gap-8">
                            <div className="lg:col-span-5 space-y-6">
                                ...
                                <div className="space-y-3 max-h-[700px] overflow-y-auto no-scrollbar pr-2">
                                    ...
                                    {filteredUsers.length === 0 && !isLoadingUsers && <div className="text-center py-20 text-zinc-600 font-black uppercase tracking-widest">No users found in database</div>}
                                </div>
                            </div>
                            <div className="lg:col-span-7">
                                {selectedUserForSticker ? (
                                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className={`${glass} p-8 lg:p-12 rounded-[4rem] border-white/10 space-y-12 shadow-2xl relative overflow-visible`}>
                                        <div className="flex items-center gap-6 pb-8 border-b border-white/10">
                                            ...
                                            <div className="flex-1">
                                              <span className="px-3 py-1 bg-[#004aad] text-white text-[9px] font-black uppercase rounded-full mb-2 inline-block">Management Panel</span>
                                        ...
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-3"><Award className="w-4 h-4 text-[#a78bfa]" /><h4 className="text-xs font-black uppercase tracking-widest text-[#a78bfa]">Grant Achievements</h4></div>
                                        ...
                                        {/* 컬렉티브 지급 그리드 */}
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-3"><Sparkles className="w-4 h-4 text-[#fbbf24]" /><h4 className="text-xs font-black uppercase tracking-widest text-[#fbbf24]">Grant Collections</h4></div>
                                            <div className="grid grid-cols-2 gap-3">
                                                {Object.entries(COLLECTIVE_DATA).map(([id, data]) => {
                                                    const hasIt = selectedUserForSticker.rewards?.includes(id);
                                                    return (
                                                        <button key={id} onClick={() => handleStickerToggle(selectedUserForSticker.id, id, !hasIt)} className={`p-5 rounded-3xl border flex items-center gap-4 transition-all text-left ${hasIt ? 'bg-[#fbbf24]/20 border-[#fbbf24] text-white shadow-[0_0_15px_rgba(251,191,36,0.2)]' : 'bg-white/5 border-white/10 text-zinc-500 hover:border-white/30'}`}>
                                                            <data.icon size={20} style={{ color: hasIt ? data.color : 'inherit' }} />
                                                            <div className="min-w-0"><p className="text-[11px] font-black uppercase truncate">{data.title}</p><p className="text-[8px] opacity-60 font-bold uppercase">{hasIt ? 'Reward Granted' : 'Click to Grant'}</p></div>
                                                        </button>
                                                    );
                                                })}
                                            </div>
                                    ...
                                    <div className={`${glass} p-20 rounded-[4rem] flex flex-col items-center justify-center text-center space-y-6 border-dashed border-white/10`}><Users size={64} className="text-zinc-800" /><p className="text-xs text-zinc-600 font-black uppercase tracking-widest">Select a user to manage records and achievements</p></div>
                                )}
                            </div>
                        </motion.div>
                    )}
                        ...
                        <motion.div key="config" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                            <div className="bg-zinc-900 border border-white/10 p-8 lg:p-12 rounded-[4rem] shadow-2xl relative overflow-hidden">
                                ...
                                <div className="grid lg:grid-cols-2 gap-16">
                                    ...
                                    <div className="space-y-8">
                                        <h3 className="text-xl font-black uppercase text-white flex items-center gap-3"><Sparkles className="w-5 h-5 text-[#004aad]"/> Intro Page (Popup Context)</h3>
                                        <div className="space-y-5">
                                            <div><p className="text-[10px] text-zinc-500 uppercase font-black mb-1.5 ml-1">Main Heading</p><input value={siteConfig.intro_title} onChange={e => setSiteConfig({...siteConfig, intro_title: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white font-bold outline-none focus:border-[#004aad]" /></div>
                                            <div><p className="text-[10px] text-zinc-500 uppercase font-black mb-1.5 ml-1">Sub Description (Popup Body)</p><textarea value={siteConfig.intro_desc} onChange={e => setSiteConfig({...siteConfig, intro_desc: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white outline-none focus:border-[#004aad] h-24 resize-none" /></div>
                            ...
                            <div className={`${glass} p-8 lg:p-12 rounded-[4rem] space-y-10`}>
                                ...
                                <div className="grid lg:grid-cols-2 gap-10">
                                    <div className="space-y-5">
                                        <div><p className="text-[10px] text-zinc-500 uppercase font-black mb-1.5 ml-1">Main Headline</p><input value={featuredData.headline} onChange={e => setFeaturedData({...featuredData, headline: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white outline-none focus:border-[#004aad]" /></div>
                                    ...
                                    <div className="space-y-5">
                                        <div><p className="text-[10px] text-zinc-500 uppercase font-black mb-1.5 ml-1">Featured Quote</p><input value={featuredData.quote} onChange={e => setFeaturedData({...featuredData, quote: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-white italic outline-none focus:border-[#004aad]" /></div>
                                        <div><p className="text-[10px] text-zinc-500 uppercase font-black mb-1.5 ml-1">Connect Track</p><select value={featuredData.linkedTrackId} onChange={e => setFeaturedData({...featuredData, linkedTrackId: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-[#004aad] font-black outline-none cursor-pointer"><option value="">-- SELECT TARGET ARTIFACT --</option>{tracks.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}</select></div>
                                    </div>
                                </div>
                            </div>
                            <button onClick={handleSaveAllConfig} className="w-full bg-[#004aad] text-white py-7 rounded-[2rem] font-black uppercase tracking-widest hover:bg-white hover:text-[#004aad] transition-all flex items-center justify-center gap-3 shadow-2xl border-2 border-transparent hover:border-[#004aad]"><Save size={24} /> Deploy Configuration and Sync Data</button>
                        </motion.div>
                    )}
                                ...
                                {tracks.map(t => (
                                  <div key={t.id} className={`${glass} p-6 rounded-4xl flex justify-between items-center group shadow-lg ${t.isHidden ? 'opacity-50 border-dashed' : 'border-white/5'}`}>
                                    <div className="flex-1 min-w-0 pr-4">
                                      <div className="flex items-center gap-2 mb-1">{t.isHidden && <span className="px-2 py-0.5 bg-red-500/20 text-red-500 text-[10px] rounded font-bold uppercase">Hidden</span>}<p className="font-black uppercase text-xl truncate">{t.title}</p></div>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      ...
                                      <button onClick={() => { if(window.confirm('정말 삭제하시겠습니까?')) deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'tracks', t.id)) }} className="p-3 text-red-500 bg-red-500/5 hover:bg-red-500/10 rounded-full"><Trash2 className="w-5 h-5" /></button>
                                    </div>
                                  </div>
                                ))}
                            </div>
                            <div className="lg:col-span-5">
                              <div className={`p-8 lg:p-12 rounded-[3rem] text-black shadow-2xl transition-all sticky top-32 ${editingId ? 'bg-emerald-400' : 'bg-white'}`}>
                                <div className="mb-8 flex items-center justify-between"><h3 className="font-black uppercase tracking-tighter text-2xl lg:text-3xl">{editingId ? 'Modify Artifact' : 'New Artifact'}</h3>{editingId && <button onClick={handleCancelEdit} className="text-xs font-black uppercase bg-black/10 px-4 py-2 rounded-full">Cancel</button>}</div>
                                <form onSubmit={handleAddOrUpdateTrack} className="space-y-6">
                                  ...
                                  <div className="space-y-2">
                                    <div className="flex justify-between items-end px-2"><span className="text-[10px] font-black uppercase tracking-widest text-black/60">Lyrics (LRC Format)</span><label className="cursor-pointer flex items-center gap-1 bg-black/10 px-3 py-1.5 rounded-full hover:bg-black/20 transition-colors"><FileText className="w-3 h-3 text-black/70" /><span className="text-[9px] font-black uppercase tracking-widest text-black/70">.LRC File</span><input type="file" accept=".lrc,.txt" onChange={handleLrcUpload} className="hidden" /></label></div>
                                    <textarea placeholder="Paste lyrics data..." value={newTrack.lyrics} onChange={e => setNewTrack({...newTrack, lyrics: e.target.value})} className="w-full bg-black/5 border-b-2 border-black/10 p-4 font-medium text-xs outline-none focus:border-black h-32 resize-none leading-relaxed font-mono rounded-xl" wrap="off" />
                                  </div>
                                  <button type="submit" disabled={isUploadingImg} className="w-full bg-black text-white py-6 mt-6 rounded-4xl font-black uppercase tracking-widest text-sm shadow-2xl disabled:opacity-50">{editingId ? 'Confirm Edit' : 'Sync New Artifact'}</button>
                                </form>
                              </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div key="playlists" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid lg:grid-cols-12 gap-8">
                            <div className="lg:col-span-7 space-y-4">
                                {playlists.map(pl => (
                                  <div key={pl.id} className={`${glass} p-6 rounded-4xl flex justify-between items-center group shadow-lg border-white/5`}>
                                    <div className="flex items-center gap-4"><div className="w-12 h-12 rounded-xl bg-zinc-800 overflow-hidden"><img src={pl.image} className="w-full h-full object-cover" /></div><div className="min-w-0 pr-4"><p className="font-black uppercase text-xl truncate">{pl.title}</p><p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">{pl.trackIds?.length || 0} Tracks</p></div></div>
                                    <div className="flex items-center gap-2 shrink-0">
                            ...
                            <div className="lg:col-span-5">
                                <div className="p-8 lg:p-12 rounded-[3rem] bg-zinc-900 border border-white/10 text-white shadow-2xl sticky top-32">
                                    <h3 className="font-black uppercase tracking-tighter text-2xl mb-8">{editingPlaylistId ? 'Update Playlist' : 'New Playlist Collection'}</h3>
                                    <form onSubmit={handleAddOrUpdatePlaylist} className="space-y-6">
                                        ...
                                        <div className="relative group cursor-pointer"><input type="file" accept="image/*" onChange={handlePLImageUpload} className="absolute inset-0 opacity-0 cursor-pointer z-10" /><div className="p-6 rounded-xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-3">{isUploadingPLImg ? <Loader2 className="w-6 h-6 animate-spin text-[#004aad]" /> : <Upload className="w-6 h-6 text-white/40" />}<span className="text-[10px] font-black uppercase tracking-widest text-white/40">Upload Playlist Cover</span></div></div>
                                        {newPlaylist.image && <div className="w-24 h-24 rounded-2xl overflow-hidden border border-white/20 mx-auto shadow-xl"><img src={newPlaylist.image} className="w-full h-full object-cover" /></div>}
                                        <div className="space-y-3">
                                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#004aad] ml-1">Select Tracks to Include</p>
                                            <div className="h-64 overflow-y-auto no-scrollbar bg-black/40 rounded-2xl border border-white/5 p-4 space-y-2">
                                                {tracks.map(t => (
                                                    <div key={t.id} onClick={() => toggleTrackInPL(t.id)} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${newPlaylist.trackIds.includes(t.id) ? 'bg-[#004aad]/20 border border-[#004aad]/50 shadow-[0_0_15px_rgba(0,74,173,0.2)]' : 'hover:bg-white/5 border border-transparent'}`}>
                                                        <div className="w-8 h-8 rounded bg-zinc-800 shrink-0 overflow-hidden shadow-lg"><img src={t.image} className="w-full h-full object-cover" /></div>
                                        ...
                                        <button type="submit" className="w-full bg-white text-black py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-[#004aad] hover:text-white transition-all shadow-xl">{editingPlaylistId ? 'Save Changes' : 'Create Collection'}</button>
                                        {editingPlaylistId && <button type="button" onClick={() => { setEditingPlaylistId(null); setNewPlaylist({ title: '', desc: '', image: '', trackIds: [] }); }} className="w-full text-[10px] font-black uppercase text-zinc-500 mt-4 hover:text-white">Cancel Edit</button>}
                                    </form>
                                </div>
                            </div>
                        </motion.div>
