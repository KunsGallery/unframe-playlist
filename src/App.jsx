...
export default function App() {
  ...
  const handleShare = async (e, item, type = 'track') => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "Unframe UP", text: "Check this sound.", url: shareUrl }); return; } catch (err) {}
    }
    try {
      const copyArea = document.createElement("textarea");
      copyArea.value = shareUrl; copyArea.style.position = "fixed"; copyArea.style.left = "-9999px"; document.body.appendChild(copyArea);
      copyArea.select(); document.execCommand('copy'); document.body.removeChild(copyArea);
      setCopiedId(type === 'reward' ? 'reward' : item.id);
      setToastMessage("링크가 복사되었습니다 ✨");
      setTimeout(() => { setCopiedId(null); setToastMessage(null); }, 3000);
    } catch (err) { setAuthError("복사 실패"); }
  };
